...
export function initializeDatabase() {
	...
	db = new sqlite3.Database(dbPath, (err) => {
		if (err) {
			...
			db.get("SELECT name FROM sqlite_master WHERE type='table' AND name='usuario'", (e, row) => {
				if (e) {
					console.error('Error verificando esquema SQLite:', e);
					return;
				}
				if (!row) {
					const statements = [
					...
					// Lecturas de frecuencia cardíaca (últimos 7 días)
					...
					// Lecturas de pasos (últimos 30 días para cálculo de incremento)
					`INSERT OR IGNORE INTO lectura (sensor_id, valor, fecha_hora)
					...
					// Más lecturas para el periodo anterior (31-60 días atrás) para el cálculo de incremento
					// Sensor ID 3 (frecuencia cardíaca Garmin)
					`INSERT OR IGNORE INTO lectura (sensor_id, valor, fecha_hora)
					...
					// Lecturas recientes para sensor 3 (últimos 30 días)
					`INSERT OR IGNORE INTO lectura (sensor_id, valor, fecha_hora)
					...
					db.serialize(() => {
						for (const s of statements) {
							db.run(s, (se) => {
								if (se) console.error('Error inicializando SQLite:', se);
							});
						}
					});
				}
